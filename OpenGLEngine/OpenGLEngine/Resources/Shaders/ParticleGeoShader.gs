...
void main() {
	...
	vec3 p1 = gl_in[0].gl_Position.xyz +(-vQuad1-vQuad2)* 0.5;
	gl_Position = vp * vec4(p1, 1.0);
	gs_out.texcoord = vec2(0.0, 0.0); EmitVertex();
	
	vec3 p2 = gl_in[0].gl_Position.xyz + (-vQuad1+vQuad2)*  0.5;
	gl_Position = vp * vec4(p2, 1.0);
	gs_out.texcoord = vec2(0.0f, 1.0); EmitVertex();
	
	vec3 p3 = gl_in[0].gl_Position.xyz + (vQuad1-vQuad2)* 0.5;
	gl_Position = vp * vec4(p3, 1.0);
	gs_out.texcoord = vec2(1.0, 0.0); EmitVertex();
	
	vec3 p4 = gl_in[0].gl_Position.xyz + (vQuad1+vQuad2)*  0.5;
