...
ssAnimatedModel::ssAnimatedModel(std::string modelFilname, std::string texFilename) {
	...
	program = ShaderLoader::GetInstance()->GetProgram((char*)"Animated");
...
void ssAnimatedModel::setShaderEffectVariables(float dt, Terrain* terrain) {
	...
	glUniform3f(cameraPosLoc, Camera::GetInstance()->GetCamPos().x, Camera::GetInstance()->GetCamPos().y, Camera::GetInstance()->GetCamPos().z);
...
void ssAnimatedModel::Process(float _deltaTime)
{
...
glm::vec3 ssAnimatedModel::getPosition() {
	return this->position;
}

glm::vec3 ssAnimatedModel::getRotation() {
	return this->rotation;
}

glm::vec3 ssAnimatedModel::getScale() {
	return this->scale;
}
